...
import config
from calculate import robot_to_screen_pixels, get_pix_per_mm_for_camera_height, cube_get_center_pixel
...
def draw_grid(output_frame, cam_center_mm, cam_angle_degrees):
    ...
    for x in range(start_x, end_x, step):
        screen_coord_start = robot_to_screen_pixels(cam_center_mm, cam_angle_degrees, (x, end_y), True)
    ...
    for y in range(start_y, end_y, step):
        ...
        screen_coord_end = robot_to_screen_pixels(cam_center_mm, cam_angle_degrees, (start_x - step, y), True)
        cv2.line(output_frame, screen_coord_start, screen_coord_end, color, 1, cv2.LINE_AA)
    #
...
def find_closest_box(boxes, target_x, target_y):
    ...
    for xyxy in boxes:
        ...
        center_x, center_y=cube_get_center_pixel(xyxy)
